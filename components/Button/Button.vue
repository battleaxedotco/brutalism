...
export default {
  ...
  mounted() {
    if (/button-group/i.test(this.$parent.$vnode.tag)) {
      this.groupItem = true;
      this.passClick = !this.$parent.multiselect && !this.$parent.exclusive;
    }
    if (this.hasTooltip) {
  ...
  methods: {
    ...
    checkTooltipPos() {
      ...
      setTimeout(() => {
        ...
        if (tooltip.style.left == "0px" && tooltip.style.right == "0px") {
          tooltip.style.left = "";
          tooltip.style.right = "";
          ...
          if (tooltip.style.right) {
            ...
            if (+rightpos >= window.innerWidth) tooltip.style.right = "";
          }
        }
