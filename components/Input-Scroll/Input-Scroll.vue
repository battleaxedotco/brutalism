...
export default {
  ...
  methods: {
    ...
    validate(value) {
      if (("" + value).length < 1 && (this.resetValue || this.resetValue == 0))
    ...
    clamp(result) {
      ...
      result =
        (this.max || this.max == 0) && result > this.max ? this.max : result;
      return result;
    },
    focus(evt = null) {
    ...
    resize(padding = 5) {
      if (padding.data && (padding.data == "." || padding.key == "Period"))
        padding = 5;
      else if (padding.data) padding = 5;
      if (arguments.length < 1 || typeof padding !== "number") padding = 5;
      this.$nextTick(() => {
...
.input-scroll-container.readOnly {
  pointer-events: none;
}
