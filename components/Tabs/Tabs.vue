<template>
  ...
  >
    <div
      :class="['active-line', { filled, flat, isGradient }]"
      :style="getActiveLineStyle()"
    ></div>
    <div
      v-for="(tab, i) in tabs"
      :class="[
  ...
  methods: {
    ...
    getActiveLineStyle() {
      ...
      if (!this.filled && !this.disabled) {
        style = `width: ${this.slideWidth}px; left: ${this.slideLeft}px; ${
          this.invert ? "top" : "bottom"
        }: ${this.slideTop}px;`;
        style += !this.showSlider ? "display: none;" : "display: block;";
        style += this.delay ? `transition-delay: ${this.delay};` : "";
        style += this.duration ? `transition-duration: ${this.duration};` : "";
        style += this.timing
...
.tab-wrapper.idle:not(.flat) > .tab-line {
  ...
  /* 
		@@@ FIX THIS 
		On ILST appears 1px off size, but interrupts scrollbar when transparent.
	*/
}
