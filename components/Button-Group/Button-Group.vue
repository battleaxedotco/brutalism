<template>
  <div class="button-group-container" :style="[{ 'margin': margin, 'width': width }]">
    <span
      ...
      :style="[{ 'margin': computedMargin, 'order': right ? 1 : 0 }]"
      v-if="label.length"
    >{{label}}</span>
    <div
      :class="['button-group', direction, !grid ? right ? 'right' : center ? 'center' : 'left' : '']"
      :style="getStyle()"
    >
      <slot />
    </div>
...
export default {
  ...
  computed: {
    ...
    computedMargin() {
      return this.right
        ? `0px 0px 0px ${this.labelMargin}`
        : `0px ${this.labelMargin} 0px 0px`;
    }
  },
  methods: {
    setActiveByUID(uid) {
    ...
    getGridStyle() {
      ...
      style += `${
        this.direction == "row" ? "grid-template-columns" : "grid-template-rows"
      }: ${this.templateString}`;
      return style;
    },
    resetChildren(uid) {
